...
from model import TinyBERTDeepCoNN
from utils import TinyBERTDeepCoNNDataset, predict_mse, date


def train(train_dataloader, valid_dataloader, model, config, model_path):
    print(f'{date()}## Start the training!')
    train_mse = predict_mse(train_dataloader, model, config.device)
    valid_mse = predict_mse(valid_dataloader, model, config.device)
    ...
    for epoch in range(config.train_epochs):
        ...
        for batch in train_dataloader:
            u_ids, u_mask, i_ids, i_mask, ratings = map(lambda x: x.to(config.device), batch)
            predict = model(u_ids, u_mask, i_ids, i_mask)
...
def test(dataloader, model, config):
    ...
    start_time = time.perf_counter()
    test_loss = predict_mse(dataloader, model, config.device)
    end_time = time.perf_counter()
...
if __name__ == '__main__':
    config = Config()
    print(config)
    print(f'{date()}## Load data...')

    train_dataset = TinyBERTDeepCoNNDataset(config.train_file, config)
    valid_dataset = TinyBERTDeepCoNNDataset(config.valid_file, config)
    test_dataset  = TinyBERTDeepCoNNDataset(config.test_file, config)
    train_dlr = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, num_workers=2)
    valid_dlr = DataLoader(valid_dataset, batch_size=config.batch_size, num_workers=2)
    test_dlr = DataLoader(test_dataset, batch_size=config.batch_size, num_workers=2)

    model = TinyBERTDeepCoNN(config).to(config.device)
